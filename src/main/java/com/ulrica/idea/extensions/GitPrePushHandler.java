...
public class GitPrePushHandler implements PrePushHandler {
    ...
    public Result handle(@NotNull List<PushInfo> list, @NotNull ProgressIndicator progressIndicator) {
        ...
        PropertiesDetail settingPersistent = PropertiesConfigurable.readProperties(currentProject);
        assert settingPersistent != null;
        if (!settingPersistent.isGitPushSwitch()) {
            LOG.info("gitPushSwitch is not open");
            return Result.OK;
        }
        String listenerDir = settingPersistent.getListenDirs();
        String exportDir = settingPersistent.getExportDirs();
